...
`CardView` — это абстрактное представление карточки, которое адаптируется под контекст отображения: на главной странице, в модальном окне или в корзине. Он переиспользуется внутри других компонентов, например, в `ProductCardView`.
